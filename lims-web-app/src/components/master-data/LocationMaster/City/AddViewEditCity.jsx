import React, { useEffect, useState } from 'react'
import Swal from '../../../Re-usable-components/Swal';
import Button from '../../../Re-usable-components/Button';
import InputField from '../../../Homepage/InputField';
import { useLocation, useNavigate, useParams } from 'react-router-dom';
import { locationMasterService } from '../../../../services/locationMasterService';
import '../City/AddViewEditCity.css'

const AddViewEditCity = () => {
  const [cityData, setCityData] = useState({
    cityName: '',
    stateName: '',
});
const [viewMode, setViewMode] = useState(false);
const [popupConfig, setPopupConfig] = useState(null);
const { id } = useParams();
const location = useLocation();
const navigate = useNavigate();

useEffect(() => {
    const cityDetails = location.state?.cityDetails;
    const mode = location.state?.mode;

    if (cityDetails) {
        const data = {
          cityName: cityDetails.cityName,
            stateName: cityDetails.stateName,
        };

        setCityData((prev) => {
            return { ...prev, ...data };
        });
    }
    
    if (mode === 'view') {
        setViewMode(true);
    }
}, [location.state]);

const validateInput = () => {
    if (
      cityData.cityName.trim() === '' || 
      cityData.stateName.trim() === '' 
    )  {
    setPopupConfig({
        icon: 'delete',
        title: 'Validation Error',
        text: 'All fields are required and cannot be empty or just spaces.',
        onClose: () => setPopupConfig(null),
    });
    return false;
    }
    return true;
};


const handleChange = (e) => {
    setCityData((prev) => ({ ...prev, [e.target.name]: e.target.value }));
}

const handleAdd = async () => {
    if (!validateInput()) return;

    try {
        const createdBy = "3fa85f64-5717-4562-b3fc-2c963f66afa7"; // Replace with the actual createdBy value
        // const labId = "5b680e6a-e0ea-4a0e-af86-1219694d3c60"; // Replace with the actual labId value

        const response = await locationMasterService.saveCity(
            {
              cityName: cityData.cityName,
              stateName: cityData.stateName
            },
            createdBy
        );

        setPopupConfig({
            icon: 'success',
            title: 'Added Successfully',
            text: '',
            onClose: () => navigate('/masters/location/city'),
          });
        } catch (error) {
          console.error(error);
          setPopupConfig({
            icon: 'delete',
            title: 'Failed to add city.',
            text: 'Please try again.',
            onButtonClick: () => setPopupConfig(null),
            onClose: () => setPopupConfig(null),
          });
        }
};

const handleNavigate = () => {
    navigate('/masters/location/city');
  }

  const handleUpdate = async () => {
    if(!validateInput()) return;
     try {
        // const id = "3fa85f64-5717-4562-b3fc-2c963f66afa7"; // Replace with the actual createdBy value
          const modifiedBy = "5b680e6a-e0ea-4a0e-af86-1219694d3c60"; // Just for the referene will remove later
          const response = await locationMasterService.updateCityById(id, cityData, modifiedBy);
          if (response.statusCode === "200 OK") {
            setPopupConfig({
              icon: 'success',
              title: 'Updated Successfully!',
              text: '',
              onClose: () => navigate('/masters/location/city'),
            });
          } else {
            setPopupConfig({
              icon: 'delete',
              title: 'Failed to update city.',
              text: 'Please try again.',
              onClose: () => navigate('/masters/location/city'),
            });
          }
        } catch (error) {
    
}}

return (
    <div className='city'>
        <div className='city-heading'>
            <span className='city-title'>{viewMode
              ? 'View City'
              : location.state?.mode === 'edit'
              ? 'Edit City'
              : 'Add City'}</span>
        </div>

        <div className='city-container'>
            <div className='city-input'>
                <InputField
                    label="City Name"
                    type="text"
                    name="cityName"
                    className="input-field-city"
                    value={cityData.cityName}
                    onChange={handleChange}
                    required
                    placeholder="Enter City name"
                    disabled={viewMode}
                    readonly={viewMode}
                />
            </div>

            <div className='city-input'>
                <InputField
                    label="State Name"
                    type="text"
                    name="stateName"
                    className="input-field-city"
                    value={cityData.stateName}
                    onChange={handleChange}
                    required
                    placeholder="Enter State Name"
                    disabled={viewMode}
                    readonly={viewMode}
                />
            </div>

        </div>

        <div className='button-container'>
        {viewMode ? (
        <div className="button-div">
          <Button text="Back" onClick={handleNavigate} />
        </div>
      ) : location.state?.mode === 'edit' ? (
        <div className="button-div">
          <Button text="Back" onClick={handleNavigate} />
          <Button text="Update" onClick={handleUpdate} />
        </div>
      ) : (
        <div className="button-div">
          <Button text="Back" onClick={handleNavigate} />
          <Button text="Save" onClick={handleAdd} />
        </div>
      )}
        </div>

        {/* Show popup if popupConfig is set */}
      {popupConfig && (
        <Swal
          icon={popupConfig.icon}
          title={popupConfig.title}
          text={popupConfig.text}
          onButtonClick={popupConfig.onButtonClick}
          onClose={popupConfig.onClose}
        />
      )}
    </div>
);
}

export default AddViewEditCity